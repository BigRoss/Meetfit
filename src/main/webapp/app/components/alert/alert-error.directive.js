(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('meetFitApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-meetFitApp-error');
                var entityKey = httpResponse.headers('X-meetFitApp-params');
                if (errorHeader) {
